...
class CertifiedCompanyFilter(django_filters.FilterSet):
    ...
    service_type = django_filters.ChoiceFilter(field_name='service_type__service', choices=Service.objects.all().values_list('service', 'service').distinct(), label='Тип услуги')
    observer = django_filters.ChoiceFilter(field_name='observer__fullname', choices=Observer.objects.all().values_list('fullname', 'fullname').distinct(), label='Наблюдатель')
